# FlowCrystal: 统一记忆系统 (Universal Memory System, UMS) 最终技术规格书
...
### 1.1 系统愿景
FlowCrystal 记忆系统不仅仅是一个存储库，它是一个**“语境桥梁”**。它通过打破即时通讯 (IM) 的碎片化与知识管理 (KM) 的静态化，实现信息的双向流动与自动结晶。
...
### 3.1 跨板块召回 (Cross-FlowCrystal Recall)
