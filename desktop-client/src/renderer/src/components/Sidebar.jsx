  ...
  return (
    <aside className="glass-panel w-[230px] h-full flex flex-col shrink-0 select-none relative z-10 shadow-xl overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="space-y-0.5">
                {['general', 'dev-core', 'design', 'ai-brain'].map(ch => (
                  <button key={ch} onClick={() => onChannelSelect(ch)}
                    className={`w-full text-left px-3 py-2 flex items-center gap-2 relative transition-all duration-300 rounded-lg group ${
                      activeChannel === ch ? 'bg-white/5 text-white' : 'text-white/40 hover:text-white/70'
                    }`}
                  >
                    {activeChannel === ch && (
                      <div className="absolute left-0 w-[2px] h-4 bg-[#38BDF8] shadow-[0_0_8px_#38BDF8] rounded-full" />
                    )}
                    <span className={`text-lg leading-none ${activeChannel === ch ? 'text-[#38BDF8]' : 'text-white/10'}`}>#</span>
                    <span className="text-[13px] font-medium tracking-tight truncate">{ch}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="px-4 pt-6 flex-1 overflow-y-auto custom-scrollbar">
              <div className="px-1 mb-3 flex items-center justify-between">
                <span className="text-[9px] font-bold text-white/20 tracking-[0.3em] uppercase">Synchronized</span>
                <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-green-500/5 border border-green-500/10">
                  <div className="w-1 h-1 bg-green-500 rounded-full animate-pulse" />
                  <span className="text-[9px] text-green-500/60 font-bold uppercase">{onlineUsers.length}</span>
                </div>
              </div>
              <div className="space-y-1">
                {onlineUsers.map(user => {
                  ...
                  return (
                    <div key={user} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-white/[0.03] transition-all group">
                      <div className="w-7 h-7 rounded-[10px] bg-gradient-to-br from-[#38BDF8] to-[#818CF8] flex items-center justify-center text-[11px] font-bold text-white shadow-sm ring-1 ring-white/10 group-hover:ring-[#38BDF8]/40 transition-all">
                        {user[0].toUpperCase()}
                      </div>
                      <div className="flex flex-col min-w-0">
                        <span className="text-[13px] text-white/60 font-medium truncate group-hover:text-white/90 transition-colors tracking-tight">
                          {user.toLowerCase()}
                        </span>
                        {isMe && <span className="text-[9px] text-[#38BDF8]/40 font-mono uppercase tracking-[0.1em] -mt-0.5">Primary Agent</span>}
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
             ...
             <div className="px-4 py-3">
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full bg-white/[0.02] border border-white/5 rounded-xl py-2 pl-9 pr-3 text-[12px] text-white placeholder:text-white/10 focus:outline-none focus:border-[#C084FC]/30 transition-all"
                  value={searchQuery}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              <div className="px-5 mb-3 flex items-center justify-between">
                <span className="text-[9px] font-bold text-white/20 tracking-[0.4em] uppercase">Core Archive</span>
                <button onClick={onCreateCrystal} className="text-white/10 hover:text-[#C084FC] transition-colors"><Plus size={14} /></button>
              </div>
      ...
      <div className="p-4 mt-auto border-t border-white/[0.03]">
        <div className="flex items-center gap-3 p-3 rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] transition-all group">
          <div className="w-9 h-9 rounded-[12px] bg-gradient-to-br from-white/10 to-white/[0.03] flex items-center justify-center text-[12px] font-bold text-white/60 shadow-inner border border-white/5 group-hover:border-white/20 transition-all">
            {currentUser?.[0]?.toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-[14px] font-bold text-white/90 truncate tracking-tight">{currentUser?.toLowerCase()}</div>
            <div className="text-[9px] text-white/20 tracking-[0.2em] uppercase font-mono mt-0.5">Authorized Node</div>
          </div>
        </div>
      </div>
    </aside>
